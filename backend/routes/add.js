...
// BALANS TƏYİN ET (admin paneldən)
router.post("/set-balance", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Balance and income updated", balance: user.balance });
  } catch (err) {
...
router.post("/add-expense", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Expense added successfully", balance: user.balance });
  } catch (err) {
...
router.post("/statistics", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const filterByDate = (list) =>
      list.filter((item) => new Date(item.date) >= start);
